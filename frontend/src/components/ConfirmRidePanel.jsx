...
const ConfirmRidePanel = (props) => {
    ...
    return (
        <div >
            ...
            <h3 className='text-2xl mb-3 text-semibold'>Confirm this ride to start</h3>
            ...
            <div className='flex gap-2 flex-col justify-center items-center'>
                ...
                <div className='flex flex-col mt-6 gap-5 justify-right w-full' >
                    <form onSubmit={(e) => { submitHandle(e) }}>
                        <input  
                        value={otp}
                        onChange={(e) => { setOtp(e.target.value) }}
                        type="text " className='w-full px-10 py-3 text-base rounded-lg bg-[#eee]' placeholder='Enter OTP' />
                        <div className='flex mt-7 flex-row gap-5 justify-right w-full'>
                            <Link
                                to='/captain-riding'
                                onClick={() => {
                                    props.setRidePopUPPanel(false)
                                }}
                                className='px-5 py-3 flex justify-center bg-green-500 w-full text-white font-semibold  rounded-xl'>Accept </Link>
                            <button
                                onClick={() => {
                                    props.setRidePopUPPanel(true)
                                    props.setConfirmedRidePanel(false)
                                }}
                                className='px-5 py-3 bg-red-500 w-full text-white font-semibold  rounded-xl'>Cancel  </button>
                        </div>
                    </form>
